...
//************************************************************************************************************************
void DibujarArchivo(int &ancho, int &alto ,const char *direccion ,int posX ,int posY ,Color colFondo ,Color colTex ,bool espacios)
{
    ancho = alto =0;
    WORD colorOriginal=ObtenerColorOriginal();
    EstablecerColor(colFondo,colTex);
    ...
    for(int i=0;!archivo.eof();i++,alto++)
    {
        ...
        for(;texto[j];j++)
        {
            ...
            {
                gotoxy(posX+j+1,posY+i);
            }
        }
        ancho = ancho > j ? ancho = ancho : ancho = j;
    }
...
//************************************************************************************************************************
void DibujarArchivoAColor(int &ancho, int &alto ,const char *direccion ,int posX ,int posY ,Color colFondo ,Color colTex)
{
    int numFon,numTex;
    ancho = alto =0;
    int an=0;
    ...
    for(int i=0;!archivo.eof();i++,alto++)
    {
        ...
        for(int j=0;texto[j];j++)
        {
            ...
            if((int)texto[j]== 3 )//Caracter que indica color
            {
                j++;
                //ahora estamos justo donde empieza

                //los numeros van del 0 al 15, si es un numero de dos digitos entonces f. uno.
                //si el siguiente numero es de un digito, no pasa nada, de lo contrario se le suma 10
                if(texto[j+1]==',')
                    numFon = ((int)texto[j])-48;
                else
                {
                    j++;
                    numFon=10+((int)texto[j])-48;
                }
                j+=2;

                if(texto[j+1]>=48 && texto[j+1]<=57)
                {
                    j++;
                    numTex = 10 +((int)texto[j])-48;
                }
                else
                    numTex = ((int)texto[j])-48;

                j++;

                //estamos en el primer caracter, necesitamos checar si es un caracter o un espacio generado por un numero.
                if(texto[j]== 2 && texto[j+1]== 2 && texto[j+2] >= 48 && texto[j+2] <= 57)
                    j+=2;

                EstablecerColor(ConvNum2Col(numTex),ConvNum2Col(numFon));
            }

            std::cout<<texto[j];
            an++;
        }
        ancho = ancho > an ? ancho = ancho : ancho = an;
    }
...
void AnimarArchivo(const char *direccion ,double segundos ,double repeticiones ,int posX ,int posY ,Color colFondo ,Color colTex)
{
    ...
    {
        DibujarArchivo(basura2,basura,frame,posX,posY,colFondo,colTex);
        Sleep(segundos);
